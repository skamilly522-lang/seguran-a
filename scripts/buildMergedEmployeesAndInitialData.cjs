...
const nameAliases = {
  'ABRAAO AVANGELISTA DOS SANTOS': 'ABRAAO EVANGELISTA DOS SANTOS'
};

function normalizeName(n) {
  const clean = n.trim().toUpperCase();
  return nameAliases[clean] || clean;
}
...
const evaluations = [];
const actionPlans = [];
const evalNameMap = {}; // name -> { role, count, dates: [] }
...
rows.forEach((row, idx) => {
  ...
  const name = normalizeName(row[5]);

  if (!evalNameMap[name]) {
    evalNameMap[name] = { role, count: 0, dates: [] };
  }
  evalNameMap[name].count++;
  evalNameMap[name].dates.push(dateStr);
...
console.log('Unique evaluated employee names in CSV:', Object.keys(evalNameMap).length);
