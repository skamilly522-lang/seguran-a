...
// Stats per employee: name -> list of dates
const employeeEvalsMap = {};
...
rows.forEach((row, idx) => {
  ...
  if (!employeeEvalsMap[name]) {
    employeeEvalsMap[name] = [];
  }
  employeeEvalsMap[name].push({ date: dateStr, role });
  ...
  evaluations.push({
    ...
    generalNotes: totalNok > 0 
      ? `Acompanhamento GSD em rota (${plate}). ${totalNok} item(ns) não conforme(s).`
      : `Acompanhamento GSD em rota (${plate}). Operação 100% conforme.`,
    responses
  });
...
console.log(`Evaluations: ${evaluations.length}, Action Plans: ${actionPlans.length}`);

// Calculate compliance summary
const sumScore = evaluations.reduce((acc, curr) => acc + curr.score, 0);
const avgScore = parseFloat((sumScore / evaluations.length).toFixed(1));

console.log('Unit Compliance Average:', avgScore);
...
// Replace INITIAL_EVALUATIONS and INITIAL_ACTION_PLANS exports
const newEvaluationsCode = `export const INITIAL_EVALUATIONS: EvaluationRecord[] = ${JSON.stringify(evaluations, null, 2)};`;
const newActionPlansCode = `export const INITIAL_ACTION_PLANS: ActionPlan[] = ${JSON.stringify(actionPlans, null, 2)};`;

// Update INITIAL_UNITS
const newUnitsCode = `export const INITIAL_UNITS: UnitSummary[] = [
  { unitName: 'Pau Brasil Guarabira', totalEvaluations: ${evaluations.length}, complianceRate: ${avgScore}, gsdCompliance: ${avgScore}, gsaCompliance: 100, gspCompliance: 100, farolStatus: '${avgScore >= 90 ? 'VERDE' : avgScore >= 75 ? 'AMARELO' : 'VERMELHO'}', openActionPlans: ${actionPlans.length} },
];`;

// Write to realGsdData.ts
const realDataTs = `import { EvaluationRecord, ActionPlan } from '../types';\n\nexport const REAL_GSD_EVALUATIONS: EvaluationRecord[] = ${JSON.stringify(evaluations, null, 2)};\n\nexport const REAL_GSD_ACTION_PLANS: ActionPlan[] = ${JSON.stringify(actionPlans, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, '../src/data/realGsdData.ts'), realDataTs);

console.log('Updated realGsdData.ts and calculated unit metrics.');
