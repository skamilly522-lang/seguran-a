...
// Helper to split CSV line safely with quotes
...
console.log('Parsed total CSV rows:', rows.length);
...
function classifyAnswer(val) {
  ...
  if (v === 'N/A' || v === 'NA' || v.startsWith('NÃO POSSUI') || v.startsWith('NÃO HÁ') || v === 'N/A') {
    return { status: 'N_A', isCompliant: true };
  }
  // OK, Sim, Bom, Ótimo, Conforme, etc
...
rows.forEach((row, idx) => {
  const unit = row[0] || 'Pau Brasil Guarabira';
  const rawDate = row[1];
  ...
  const totalItems = totalOk + totalNok; // Non-N_A items
  ...
  const evalRecord = {
    ...
    generalNotes: totalNok > 0 
      ? `Avaliação GSD realizada em rota. Identificadas ${totalNok} inconformidades registradas em relatório.`
      : `Avaliação GSD realizada em rota. Operação 100% conforme.`,
    responses
  };

  evaluations.push(evalRecord);

  // Generate action plans for NOK items
  nokItems.forEach((nok, nokIdx) => {
    actionPlans.push({
      ...
      unit,
      itemText: nok.qText,
      problemDescription: `Inconformidade identificada no item "${nok.qText}" referente a ${name} (${role}) na rota com placa ${plate}. Resposta: ${nok.rawVal}`,
      actionRequired: `Realizar orientação técnica e alinhamento do procedimento de segurança referente a ${nok.qCat}.`,
...
console.log(`Generated ${evaluations.length} evaluation records.`);
console.log(`Generated ${actionPlans.length} action plans.`);

// Write summary to output file
fs.writeFileSync('parsed_gsd_result.json', JSON.stringify({ evaluations, actionPlans }, null, 2));
